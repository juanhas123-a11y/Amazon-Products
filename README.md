...
El archivo `validation.js` define una regla de validación (`$jsonSchema`) para asegurar la integridad de los campos críticos de la colección `products\\\_reviews`, como la existencia de `product\\\_id`, `category`, `rating` y `discounted\\\_price`.
